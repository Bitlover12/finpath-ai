# FinPath Production Policy Freeze — updated 2026-09-02
...
총 15개 상품/기간 variant, 10개 정책 프로그램을 구조화했다.
...
- 청년예술인 예술활동 적립계좌
- 희망저축계좌Ⅰ: 2026년 9월 3차 모집 중(OPEN), 월 10만원 저축 시 정부 월 30만원 지원. 전액 지급은 탈수급 등 지급요건 확인을 전제로 계산
- 청년내일저축계좌: 2026년 신규모집 종료(CLOSED), 월 10만원 저축 시 정부 월 30만원 정액 매칭 구조를 다음 모집 대비 데이터로 보존
...
8. 고정/상한형 정부매칭 정책은 지원금이 포화되는 납입액을 별도 breakpoint로 평가한다. 예: 본인 10만원에 정부 30만원이 고정되는 정책은 계좌 한도가 50만원이어도 10만원 배정과 50만원 배정을 비교한다.
9. 생계·의료급여 수급 여부처럼 핵심 자격 사실은 `preview_assumable=false`로 표시하여 조건부 Preview에서 자동으로 참이라고 가정하지 않는다.
